...
class Read {
    ...
    /*Read(const Read& other) :  localBufferAllocationOffset(0)
  {
        ...
        bool                allUpper = false)
    {
      ...
      if (! allUpper) {
        ...
        for (unsigned i = 0; i < dataLength; i++) {
          anyLowerCase |= IS_LOWER_CASE_OR_DOT[i_data[i]];
        }
        ...
        if (anyLowerCase) {
          ...
          for (unsigned i = 0; i < dataLength; i++) {
            upcaseForwardRead[i] = TO_UPPER_CASE_DOT_TO_N[i_data[i]];
          }
          sequence = BaseSeq(i_dataLength, upcaseForwardRead, false);
          unclippedData = data = BaseRef(&sequence);
        }
      }
    }
    ...
    inline BaseRef getData() {return data;}
    inline BaseRef getUnclippedData() {return unclippedData;}
    ...
    inline unsigned getFrontClippedLength() {return (unsigned)(data.getOffset() - unclippedData.getOffset());}    // number of bases clipped from the front of the read
    inline unsigned getBackClippedLength() {return unclippedLength - dataLength - getFrontClippedLength();}
  ...
  private:
    ...
    BaseRef externalData;               // The data that was passed in at init() time, memory doesn't belong to this.
...
class ReadWithOwnMemory : public Read {
  public:
    ...
    ReadWithOwnMemory(Read &baseRead) {
      set(baseRead);
    }
  ...
  private:
    ...
    void set(Read &baseRead)
    {
      ...
      const char * src = baseRead.getUnclippedData().toChars();
      memcpy(dataBuffer,src,baseRead.getUnclippedLength());
