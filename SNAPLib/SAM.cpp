    ...
    const
{
    ...
    if ((context.header == NULL || ! context.headerMatchesIndex) && context.genome != NULL && !omitSQLines) {
        ...
        for (int i = 0; i < numContigs; i++) {
            ...
            bytesConsumed += snprintf(header + bytesConsumed, headerBufferSize - bytesConsumed, "@SQ\tSN:%s\tLN:%lld\n", contigs[i].name, end - start);
    ...
    ) const
{
    ...
    *o_addFrontClipping = 0;
    ...
    int charsInString = snprintf(buffer, bufferSpace, "%.*s\t%d\t%s\t%lld\t%d\t%s\t%s\t%lld\t%lld\t%.*s\t%.*s%s%.*s%s%s\tPG:Z:SNAP%s%.*s\n",
